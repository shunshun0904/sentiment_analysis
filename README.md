...
| `FETCH_LIMIT` | `100` | 1回の更新で読む最大記事数 |
| `MAX_PAGES` | `3` | 1回の更新で繰るページ数の上限。日次のリクエスト上限を守る砦 |
...
### APITube 無料プランの制約(未確定・設計に直結)

**apitube.io / docs / support いずれもサンドボックスから到達できない**（egressポリシーで遮断）。
以下はすべて検索経由で拾った記述であり、**一次情報での裏取りができていない**。
確定できるのは、鍵を持っている本人のダッシュボードと、実際のレスポンスヘッダだけ。

| 項目 | 拾えた記述 | 確度 |
|---|---|---|
| 日次上限 | **1,000クレジット/日**（00:00 UTC リセット） | ベンダーの料金/無料枠/ヘルプの各ページが一致。**比較的堅い** |
| クレジットの単位 | 「呼び出しによって消費クレジット数が異なる」 | **1リクエスト=1クレジットとは限らない** |
| データ遅延 | 無料は12時間遅延 | 比較記事由来。**要実測**（これが本当なら5分更新は無意味） |
| 1リクエストの件数 | 無料は10件 | 同上。**要実測** |
| Webhook | 「全**有料**プランに含む」と列挙 | 無料に含まれるかは**明記なし** |
| Webhook配信が枠を消費するか | **どの資料にも記載を見つけられなかった** | **不明** |

以前ここに「100リクエスト/日」と書いていたが、その数字は他社との比較記事由来で、
ベンダー自身の料金ページ・無料枠ページ・ヘルプセンターはいずれも
**1,000クレジット/日**と書いている。1,000/日なら5分間隔(288回/日)は枠内に収まる
（ただしページ繰りとクレジット単価しだいでは足りなくなる）。

#### Webhook配信枠の件（未解決）

「Webhookの配信がAPIリクエストと同じ日次プールを食うか」は、到達できた資料の
どこにも明示がなかった。**別プールに見える、という観察は資料からは裏付けも否定もできない。**
確かめ方は2つ:

1. **ダッシュボードで見る**（最速）。Webhookの項目がそもそも無料プランに出ていなければ、
   この問いは無料プランでは成立しない（有料機能の可能性が高い）
2. **カウンタで測る**。使用量カウンタを控える → Webhookを1回配信させる → もう一度見る。
   増えていれば同一プール、増えなければ別プール

なお、**仮に別プールでもすぐには採らない**。Webhook受けにはHTTPSの公開受け口
（API Gateway + Lambda）が要り、いまのS3だけの構成より複雑になる。
「5分ポーリングが枠に収まらない」と分かってから検討する順序でよい。

#### 実測の代わりに、本番で毎回記録する

日次上限が資料で確定できないので、**レスポンスヘッダの残枠を毎回 `latest.json` に残す**
ようにしてある（`meta.api_quota` / `meta.api_requests`）。数日ぶん眺めれば、
実際の上限と消費ペースが分かる。`MAX_PAGES`（既定3）がページ繰りの上限で、
日次上限を超えないための最後の砦。
